...
> [!WARNING]  
> Ubuntu Desktop 24.04 has a [bug](https://bugs.launchpad.net/subiquity/+bug/2062988) that will make the installer crash when using an autoinstall file. The workaround is to close the installer, open a terminal and run the command `sudo snap refresh`. After this, the installer should work if launched again. This operation requires internet connection, making impossible to perform air-gapped installs with this image.
